...
if (def == 0)  && (pod == 1)
    ...
    dir='/mnt/sda2/cortes/Results/17_04/two_phases/bcspe10/';
    
    folder=[ '10-' num2str(k) '_' 'cp' num2str(cp)];
    mkdir([dir], folder)
    dir1 = [dir folder '/'];
...
cartDims = [  60,  220,   numel(layers)];
    ...
    G = cartGrid([60 220 numel(layers)]);
    G = computeGeometry(G);
    % Disable gravity
    %gravity off
    gravity on
...
%     
 ...
 %% Solve initial pressure in reservoir
   ...
   %% Transport loop
    ...
    dT     = T/60;
    dTplot = 150*day();  % plot only every 100th day
    N      = fix(T/dTplot);
    pv     = poreVolume(G,rock);
    t      = 0;
    plotNo = 1;
