...
%Create the directory
    dir='/mnt/sda2/cortes/Results/17_06/test/';
    
    folder=[ '10-' num2str(k) '_' num2str(sz) 'perm_' num2str(per)];
    mkdir([dir], folder)
    dir1 = [dir folder '/'];
...
hk=figure(234);
   ...
   plot(s, kr), legend('kr_1', 'kr_2'),title(['rel perm'])
  if savefiles==1
      if cp==1
...
for i= 1:2:8
    rock.perm(1+(i-1)*lsz*nx:i*lsz*nx)  = 100*10^(-per)*milli*darcy();
end
    figure
...
%         case 1
%             mrstModule add agmg
%             solver = AGMGSolverAD('tolerance', tol,'maxIterations', maxIterations);
%         case 2
%             solver = GMRES_ILUSolverAD('tolerance', tol,'maxIterations', maxIterations);
%         case 3
...
[N,M]    = deal(6,10);
wellSols = cell(N*M+1,1);  wellSols{1} = getWellSol(W, rSol, fluid);
oip      = zeros(N*M+1,1); oip(1) = sum(rSol.s(:,2).*pv);
for n=1:N
    fprintf(1,'Main step %d: ',n);
    for m=1:M
       rSol  = psolve(rSol);
       rSol  = implicitTransport(rSol, G, T/(N*M), rock, fluid, 'wells', W);
       p0 = rSol.pressure;
       wellSols{(n-1)*M+m+1} = getWellSol(W, rSol, fluid);
       oip((n-1)*M+m+1) = sum(rSol.s(:,2).*pv);
    ...
    contourf(reshape(G.cells.centroids(:,1), G.cartDims),...
        ...
        reshape(rSol.s(:,1),G.cartDims), [0 cval 1], 'EdgeColor','none');
    hold on;
    pause
    ...
    plot(tau(:,1)/tbf,rSol.s(:,1),'.k','MarkerSize',4);
