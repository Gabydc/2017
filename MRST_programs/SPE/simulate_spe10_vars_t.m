...
%%
x  = initState(G,W,p_0*barsa, [0 1]);
x  = incompTPFA(x, G, T, fluid, 'wells', W);

%% Extra to compute times

pv     = poreVolume(G, rock);
Time  = 1.2*sum(pv)/x.wellSol(5).flux;
 fprintf('Time necessary to inject 1.2 pv:  %1.5f [day]\n', Time/day);
...
for i=1:4
tbf(i) = tau(W(i).cells,1);
fprintf('Breakthrough time in the %02d-Producer:  %1.5f [s]\n', i, tbf(i));
